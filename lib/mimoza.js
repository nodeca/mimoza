...
// cache mime files
var mimeTypes = fs.readFileSync(path.join(__dirname, '../types/mime.types'), 'ascii').split(/[\r\n]+/);
var nodeTypes = fs.readFileSync(path.join(__dirname, '../types/node.types'), 'ascii').split(/[\r\n]+/);
 ...
 *
 *  Load an Apache2-style ".types" file, or array of strings from preloaded file
 *
...
Mimoza.prototype.loadFile = function loadFile(file) {
  ...
  if (Array.isArray(file)) {
    ...
    content = fs.readFileSync(file, 'ascii').split(/[\r\n]+/);
  }
...
Mimoza.prototype.getExtension = function getExtension(mimeType) {
  ...
  var clearMime = mimeType.split(';')[0].trim().toLowerCase();

  return this.extensions[clearMime];
};
...
builtin.loadFile(mimeTypes);


// Load additional types from node.js community
builtin.loadFile(nodeTypes);
